    ...
    <Card className="bg-white border border-gray-300 shadow-md">
      ...
      <CardContent className="space-y-4">
        {driverRequests.map((driver) => (
          <div key={driver.id} className="border border-gray-300 rounded-lg p-4">
            <div className="flex items-start justify-between mb-3">
              <div>
                <h4 className="text-black font-medium">{driver.name}</h4>
                <p className="text-gray-600 text-sm">{driver.email}</p>
                <p className="text-gray-600 text-sm">{driver.phone}</p>
              </div>
              <div className="flex items-center space-x-1">
                <Star className="w-4 h-4 text-yellow-500 fill-current" />
                <span className="text-black text-sm">{driver.rating}</span>
              </div>
            </div>
            
            <div className="mb-3">
              <p className="text-gray-800 text-sm">{driver.vehicleType}</p>
            </div>
            
            <div className="flex flex-wrap gap-2 mb-3">
              <Badge className={driver.licenseVerified ? "bg-green-600 text-white" : "bg-red-600 text-white"}>
                License: {driver.licenseVerified ? "✓" : "✗"}
              </Badge>
              <Badge className={driver.insuranceVerified ? "bg-green-600 text-white" : "bg-red-600 text-white"}>
                Insurance: {driver.insuranceVerified ? "✓" : "✗"}
              </Badge>
              <Badge
                className={
                  driver.backgroundCheck === "completed"
                    ? "bg-green-600 text-white"
                    : driver.backgroundCheck === "pending"
