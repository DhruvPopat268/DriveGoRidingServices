...
export const CancelledRidesPage = ({ onNavigateToDetail, onNavigateToRiderDetail, onNavigateToDriverDetail }) => {
  ...
  const getWhoCancelBadgeClass = (whoCancel) => {
    switch (whoCancel) {
            ...
            <table className="border-collapse" style={{ minWidth: '1400px', width: '100%' }}>
              <thead>
                <tr className="border-b border-gray-200">
                  {['Ride ID', 'Rider Info', 'Route', 'Category', 'Service Type', 'Usage', 'Partner Details', 'Date & Time', 'Status', 'Cancelled By', 'Actions'].map(col => (
              ...
              <tbody>
                {rides.map((ride) => (
                  <tr key={ride._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    {/* Cancelled By */}
                    <td className="p-3">
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${getWhoCancelBadgeClass(ride.whoCancel)}`}>
                        {ride.whoCancel || 'N/A'}
                      </span>
                    </td>
