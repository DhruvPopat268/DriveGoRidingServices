...
interface DriversPendingForPaymentPageProps {
  onNavigateToDetail?: (driverId: string) => void;
}

export const DriversPendingForPaymentPage = ({ onNavigateToDetail }: DriversPendingForPaymentPageProps) => {
  ...
  const fetchDrivers = async () => {
    try {
      const response = await apiClient.get(`${import.meta.env.VITE_API_URL}/api/driver/PendingForPayment?page=${currentPage}&limit=${recordsPerPage}`);
  ...
  return (
    <div className="p-6 space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Driver Registration Requests - Pending For Payment</CardTitle>
        </CardHeader>
        <CardContent>
