...
export default function ApprovedVehiclesPage() {
  ...
  const fetchApprovedVehicles = async () => {
    try {
      const response = await apiClient.get(`${import.meta.env.VITE_API_URL}/api/driver/vehicles/admin/approved?page=${currentPage}&limit=${recordsPerPage}`);
    ...
    return (
      <div className="space-y-8">
        <div className="flex justify-center items-center py-8">
          ...
          <span>Loading approved vehicles...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Approved Vehicles</h1>
      </div>
              ...
              {vehicles.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={9} className="text-center py-8">
                    <p className="text-gray-500 text-lg">No approved vehicles found</p>
                  </TableCell>
                </TableRow>
              ) : (
