...
import { MapPin, Clock, User, Phone, Calendar, Eye, Loader, ChevronLeft, ChevronRight, XCircle, CalendarClock } from "lucide-react";
...
interface ReachedRidesPageProps { onNavigateToDetail?: (rideId: string) => void; }
...
export const ReachedRidesPage = ({ onNavigateToDetail }: ReachedRidesPageProps) => {
  ...
  const fetchRides = async () => {
    try {
      ...
      const res = await apiClient.get(`${import.meta.env.VITE_API_URL}/api/rides/reached?${params}`);
  ...
  if (loading) return <div className="flex justify-center items-center py-8"><Loader className="w-6 h-6 animate-spin mr-2" /><span>Loading reached rides...</span></div>;
  ...
  return (
    <div className="space-y-6 bg-white text-black p-6 overflow-x-hidden">
      ...
      <Card className="bg-white border-gray-200">
        <CardHeader><CardTitle className="text-black">Reached Rides ({totalRides})</CardTitle></CardHeader>
        <div className="px-6">
        ...
        <CardContent>
              ...
              <tbody>
                {rides.map((ride) => (
                  <tr key={ride._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="p-3"><div className="text-sm font-semibold text-teal-600">{formatCurrency(ride.totalPayable)}</div></td>
          ...
          {rides.length === 0 && <div className="text-center py-8 text-gray-500">{dateFilter === 'today' ? 'No reached rides found for today' : dateFilter === 'yesterday' ? 'No reached rides found for yesterday' : 'No reached rides found'}</div>}
