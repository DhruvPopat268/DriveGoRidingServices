...
const RiderApprovedWithdrawalPage: React.FC = () => {
  ...
  const fetchApprovedRequests = async () => {
    try {
      const response = await fetch('/api/rider/withdraw?status=approved');
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Approved Rider Withdrawal Requests</h1>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p><strong>Approved:</strong> {new Date(request.updatedAt).toLocaleDateString()}</p>
                </div>
                <div>
      ...
      {requests.length === 0 && (
        <Card>
          <CardContent className="text-center py-8">
            <p>No approved withdrawal requests found.</p>
          </CardContent>
        </Card>
      )}
    </div>
