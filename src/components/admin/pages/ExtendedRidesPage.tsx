...
interface ExtendedRidesPageProps { onNavigateToDetail?: (rideId: string) => void; }

export const ExtendedRidesPage = ({ onNavigateToDetail }: ExtendedRidesPageProps) => {
  ...
  const fetchRides = async () => {
    try {
      ...
      const res = await apiClient.get(`${import.meta.env.VITE_API_URL}/api/rides/extended?${params}`);
  ...
  if (loading) return <div className="flex justify-center items-center py-8"><Loader className="w-6 h-6 animate-spin mr-2" /><span>Loading extended rides...</span></div>;
  ...
  return (
    <div className="space-y-6 bg-white text-black p-6 overflow-x-hidden">
      ...
      <Card className="bg-white border-gray-200">
        <CardHeader><CardTitle className="text-black">Extended Rides ({totalRides})</CardTitle></CardHeader>
        <div className="px-6">
        ...
        <CardContent>
              ...
              <tbody>
                {rides.map((ride) => (
                  <tr key={ride._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="p-3"><div className="text-sm font-semibold text-purple-600">{formatCurrency(ride.totalPayable)}</div></td>
          ...
          {rides.length === 0 && <div className="text-center py-8 text-gray-500">{dateFilter === 'today' ? 'No extended rides found for today' : dateFilter === 'yesterday' ? 'No extended rides found for yesterday' : 'No extended rides found'}</div>}
