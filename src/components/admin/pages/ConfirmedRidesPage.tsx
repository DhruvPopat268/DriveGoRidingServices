...
export const ConfirmedRidesPage = ({ onNavigateToDetail, onNavigateToRiderDetail, onNavigateToDriverDetail }: ConfirmedRidesPageProps) => {
  ...
  const handleRemoveDriver = async (ride) => {
    ...
    try {
      const response = await apiClient.post(`${import.meta.env.VITE_API_URL}/api/rides/eligible-drivers`, { rideId: ride._id });
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-8">
        ...
        <span>Loading confirmed rides...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 bg-white text-black p-6 overflow-x-hidden">
      ...
      <Card className="bg-white border-gray-200">
        <CardHeader>
          <CardTitle className="text-black">Confirmed Rides ({totalRides})</CardTitle>
        </CardHeader>
        <div className="px-6">
          {/* Filter Section */}
          <RideFilters
        ...
        <CardContent>
              ...
              <tbody>
                {rides.map((ride) => (
                  <tr key={ride._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    {/* Ride ID */}
                    <td className="p-3">
                      <div className="text-sm font-mono text-blue-600">{ride.bookingId || 'N/A'}</div>
                      <Badge variant={ride.bookedBy === 'STAFF' ? 'secondary' : 'default'} className="text-xs mt-1">
                        {ride.bookedBy || 'USER'}
                      </Badge>
                      {ride.city && <div className="text-xs text-gray-500 mt-0.5">{ride.city}</div>}
                    </td>
                    ...
                    <td className="p-3">
                      <div className="space-y-1">
                        <div className="text-sm text-gray-700">{ride.rideInfo?.selectedUsage || 'N/A'}</div>
                        <div className="text-sm font-semibold text-yellow-600">{formatCurrency(ride.totalPayable)}</div>
          ...
          {rides.length === 0 && (
            <div className="text-center py-8 text-gray-500">
              {dateFilter === 'today' ? 'No confirmed rides found for today' :
                dateFilter === 'yesterday' ? 'No confirmed rides found for yesterday' :
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <input type="date" value={rescheduleDate} onChange={(e) => setRescheduleDate(e.target.value)} min={new Date().toISOString().split('T')[0]} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">New Time</label>
              <input type="time" value={rescheduleTime} onChange={(e) => setRescheduleTime(e.target.value)} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
            </div>
          </div>
          <DialogFooter>
      ...
      <AdminExtraChargesDialog
        ...
        onClose={() => { setShowExtraChargesDialog(false); setSelectedRideForCharges(null); }}
