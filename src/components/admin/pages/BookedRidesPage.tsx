...
import { MapPin, Clock, User, Phone, Calendar, Eye, Loader, ChevronLeft, ChevronRight, UserPlus, XCircle, CalendarClock } from "lucide-react";
...
export const BookedRidesPage = ({ onNavigateToDetail }: BookedRidesPageProps) => {
  ...
  useEffect(() => {
    if (filterCategory && filterCategory !== 'all') {
      setFilterSubcategoriesForFilter(subCategories.filter((sub: SubCategory) => sub.categoryId === filterCategory));
  ...
  const handleDateFilter = (filter: string) => { setDateFilter(filter === dateFilter ? '' : filter); setDateRange({ from: '', to: '' }); setCurrentPage(1); };
  ...
  const handleAssignDriver = async (ride) => {
    setSelectedRide(ride); setShowAssignDialog(true);
  ...
  const confirmDriverAssignment = async () => {
    ...
    try { await apiClient.post(`${import.meta.env.VITE_API_URL}/api/rides/admin/driver/confirm`, { rideId: selectedRide._id, driverId: selectedDriver }); setShowAssignDialog(false); setSelectedDriver(''); toast.success('Driver assigned successfully!'); fetchRides(); }
  ...
  if (loading) return <div className="flex justify-center items-center py-8"><Loader className="w-6 h-6 animate-spin mr-2" /><span>Loading booked rides...</span></div>;
  ...
  return (
    <div className="space-y-6 bg-white text-black p-6 overflow-x-hidden">
      ...
      <Card className="bg-white border-gray-200">
        <CardHeader><CardTitle className="text-black">Booked Rides ({totalRides})</CardTitle></CardHeader>
        <div className="px-6">
        ...
        <CardContent>
            ...
            <table className="border-collapse" style={{ minWidth: '1200px', width: '100%' }}>
              <thead>
                <tr className="border-b border-gray-200">
                  {['Ride ID','Rider Info','Route','Service Type','Driver Category','Usage','Date & Time','Amount','Payment','Booked By','Staff Info','Actions'].map(col => (
              ...
              <tbody>
                {rides.map((ride) => (
                  <tr key={ride._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="p-3"><div className="text-sm font-semibold text-yellow-600">{formatCurrency(ride.totalPayable)}</div></td>
                    ...
                    <td className="p-3">
                      <div className="flex space-x-1">
                        ...
                        <Button size="sm" variant="outline" className="h-8 px-2 text-xs bg-white" onClick={() => handleAssignDriver(ride)} title="Assign Driver"><UserPlus className="w-4 h-4" /></Button>
          ...
          {rides.length === 0 && <div className="text-center py-8 text-gray-500">{dateFilter === 'today' ? 'No booked rides found for today' : dateFilter === 'yesterday' ? 'No booked rides found for yesterday' : 'No booked rides found'}</div>}
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-1">
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return <Button key={p} variant={currentPage === p ? "default" : "outline"} size="sm" onClick={() => handlePageChange(p)} className="w-8 h-8 p-0">{p}</Button>;
                })}
              </div>
      ...
      <Dialog open={showAssignDialog} onOpenChange={setShowAssignDialog}>
        <DialogContent>
          <DialogHeader><DialogTitle>Assign Driver to Ride</DialogTitle></DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={confirmDriverAssignment} disabled={!selectedDriver || assigningDriver}>{assigningDriver ? 'Assigning...' : 'Assign Driver'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
