...
export const ParcelRideCostPage = () => {
  ...
  // Filter parcel vehicle types when parcel category changes
  useEffect(() => {
    if (rideCostForm.parcelCategory) {
  ...
  const handleDownloadSample = async () => {
    ...
    try {
      const response = await apiClient.get(
        `${import.meta.env.VITE_API_URL}/api/ParcelRideCosts/sample-excel`,
        { responseType: 'blob' }
      );
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'parcel_packages_sample.xlsx');
  ...
  const handleExport = async () => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        ...(filterParcelCategory !== 'all' && { parcelCategory: filterParcelCategory }),
        ...(filterParcelVehicle !== 'all' && { parcelVehicle: filterParcelVehicle }),
      });
      const response = await apiClient.get(
        `${import.meta.env.VITE_API_URL}/api/ParcelRideCosts/export?${params}`,
        { responseType: 'blob' }
      );
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'parcel_packages_export.xlsx');
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
  ...
  const handleBulkImport = async () => {
    ...
    try {
      const response = await apiClient.post(
        `${import.meta.env.VITE_API_URL}/api/ParcelRideCosts/bulk-import`,
        formData,
      ...
      <Card className="p-6">
        <div className="flex items-center justify-end mb-1">
          ...
          >
            ...
            <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Bulk Import Parcel Packages</DialogTitle>
              </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-800">
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    ...
                    <li>Category, Subcategory, Parcel Category, and Parcel Vehicle names must match exactly (case-insensitive) with the names in the system</li>
                    <li>Parcel Vehicle must belong to the selected Parcel Category</li>
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>{editingRideCost ? 'Edit' : 'Create'} Parcel Package</DialogTitle>
              </DialogHeader>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    {rideCostForm.category && !isFormParcelCategory() && (
                      <p className="text-red-500 text-sm mt-1">Only parcel category is allowed</p>
                    )}
                  </div>
                  ...
                  {/* Parcel Category Selection - Only show for parcel category */}
                  {isFormParcelCategory() && rideCostForm.subcategory && (
                    <div>
                      <label className="text-sm font-medium block mb-1">Parcel Category</label>
                      <Select
                        value={rideCostForm.parcelCategory}
                        onValueChange={(value) => setRideCostForm(prev => ({ ...prev, parcelCategory: value }))}
                      ...
                      >
                        ...
                        <SelectContent>
                          {parcelCategories.map((parcelCat) => (
                            <SelectItem key={parcelCat._id} value={parcelCat._id}>
                              {parcelCat.categoryName}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                  ...
                  {/* Parcel Vehicle Selection - Only show when parcel category is selected */}
                  {isFormParcelCategory() && rideCostForm.parcelCategory && (
                    <div>
                      ...
                      <Select
                        ...
                        disabled={!rideCostForm.parcelCategory}
                        required
                      >
                        <SelectTrigger>
                        ...
                        <SelectContent>
                          {filteredParcelVehicleTypes.map((vehicle) => (
                  ...
                  {/* For non-outstation subcategories and non-parcel categories */}
                  {!isOutstationSubCategory() && rideCostForm.subcategory && !isFormParcelCategory() && (
            ...
            <div>
              ...
              <div>
                ...
                {filterCategory && filterCategory !== 'all' && (() => {
                  ...
                  return selectedCategory && selectedCategory.name.toLowerCase() !== 'parcel' && (
                    <p className="text-red-500 text-sm mt-1">Only parcel category is allowed</p>
                  );
                })()}
              </div>
        ...
        <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>View Parcel Package</DialogTitle>
            </DialogHeader>
            {viewingRideCost && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                ...
                {viewingRideCost.parcelCategory && (
                  <div>
                    <label className="text-sm font-medium">Parcel Category</label>
                    <p className="text-sm text-gray-600">{getName(viewingRideCost.parcelCategory)}</p>
                  </div>
                )}
                <div>
